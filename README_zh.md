...
*   **经验卫生机制：** 每次任务执行后，MONAD 会反思并保存经验。但失败经验会被标记为 `[FAILED]` 并在下次推理时被排除——防止"经验污染"，即过去的错误结论误导未来的推理。
...
4.  **反思 & 固化：** 任务成功后，`Reflection` 模块总结经验，`SkillBuilder` 评估是否应将其抽象为永久可复用的本地“技能”。
5.  **回答：** 基于执行验证后的真实数据，整理出最终回答。
