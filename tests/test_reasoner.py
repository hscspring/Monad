...
import pytest
from monad.cognition.reasoner import Reasoner, _thought_similarity
from monad.cognition.parser import parse_response, _normalize_parsed
from monad.cognition.hints import action_hint
from monad.cognition.prompts import build_reasoner_system, _PLATFORM_INFO
from monad.config import CONFIG

REASONER_SYSTEM = build_reasoner_system(str(CONFIG.skills_path))
...
class TestParseResponse:
    ...
    def test_pure_json_action(self):
    ...
    def test_pure_json_thought(self):
        parsed = parse_response('{"type": "thought", "content": "I need to search"}')
        assert parsed["type"] == "thought"

    def test_pure_json_answer(self):
        parsed = parse_response('{"type": "answer", "content": "Done!"}')
    ...
    def test_mixed_text_json(self):
        parsed = parse_response('思考: {"type": "thought", "content": "分析中..."}')
        assert parsed["type"] == "thought"

    def test_think_tag_stripped(self):
        parsed = parse_response('<think>internal</think>{"type": "answer", "content": "result"}')
        assert parsed["type"] == "answer"
    ...
    def test_alternative_action_format(self):
        parsed = parse_response('{"action": "shell", "params": {"command": "pwd"}}')
        assert parsed["type"] == "action" and parsed["capability"] == "shell"

    def test_alternative_capability_format(self):
        parsed = parse_response('{"capability": "web_fetch", "params": {"url": "https://x.com"}}')
        assert parsed["type"] == "action" and parsed["capability"] == "web_fetch"

    def test_alternative_answer_format(self):
        parsed = parse_response('{"answer": "final result"}')
        assert parsed["type"] == "answer" and parsed["content"] == "final result"

    def test_minimax_xml_stripped(self):
        parsed = parse_response(
            '<minimax:tool_call>junk</minimax:tool_call>{"type": "answer", "content": "ok"}')
        assert parsed["type"] == "answer"

    def test_short_string_is_error(self):
        assert parse_response("hi")["type"] == "error"
...
class TestNormalizeParsed:
    ...
    def test_already_standard(self):
        ...
        assert _normalize_parsed(d) == d

    def test_action_shorthand(self):
        result = _normalize_parsed({"action": "web_fetch", "params": {"url": "x"}})
        assert result["type"] == "action" and result["capability"] == "web_fetch"

    def test_thought_shorthand(self):
        assert _normalize_parsed({"thought": "hmm"})["type"] == "thought"

    def test_unknown_format(self):
        assert _normalize_parsed({"foo": "bar"}) is None

    def test_ask_user_type_becomes_action(self):
        result = _normalize_parsed({"type": "ask_user", "content": "你想要什么？"})
        assert result["type"] == "action"
        assert result["capability"] == "ask_user"
        assert result["params"]["question"] == "你想要什么？"

    def test_ask_user_with_question_field(self):
        result = _normalize_parsed({"type": "ask_user", "question": "请确认"})
        assert result["capability"] == "ask_user"
...
class TestThoughtSimilarity:
    ...
    def test_identical(self):
        assert _thought_similarity("hello world", "hello world") == 1.0

    def test_completely_different(self):
        assert _thought_similarity("hello world", "foo bar") == 0.0

    def test_partial_overlap(self):
        sim = _thought_similarity("the quick brown fox", "the slow brown dog")
        assert 0.0 < sim < 1.0

    def test_empty_string(self):
        assert _thought_similarity("", "hello") == 0.0
        assert _thought_similarity("", "") == 0.0
...
class TestActionHint:
    ...
    def test_open_app_hint(self):
        hint = action_hint("shell", {"command": "open -a Lark"}, "(no output)")
        assert "activate Lark" in hint
        assert "Do NOT run open -a again" in hint

    def test_open_app_error_no_hint(self):
        hint = action_hint("shell", {"command": 'open -a "Feishu"'},
                           "[stderr] Unable to find application named 'Feishu'")
        assert hint == ""

    def test_activate_hint_no_auto_screenshot(self):
        hint = action_hint("desktop_control", {"action": "activate Lark"},
                           'Activated "Lark" (verified in foreground)')
        assert "screenshot" in hint.lower()

    def test_activate_hint_with_auto_screenshot(self):
        result = ('Activated "Lark" (verified in foreground)\n'
                  '[Auto-screenshot of Feishu window] Found 20 UI elements:')
        hint = action_hint("desktop_control", {"action": "activate Lark"}, result)
        assert "click" in hint.lower() or "interact" in hint.lower()
        assert "Do NOT" in hint
        assert "desktop_control" in hint

    def test_screenshot_hint(self):
        hint = action_hint("desktop_control", {"action": "screenshot"},
                           '[Frontmost app: Lark] Found 20 UI elements (full screen):')
        assert "click" in hint.lower() or "interact" in hint.lower()

    def test_screenshot_hint_generic(self):
        hint = action_hint("desktop_control", {"action": "screenshot"},
                           '[Feishu window] Found 20 UI elements:')
        assert "desktop_control" in hint or "click" in hint.lower()

    def test_click_with_also_matched_hint(self):
        hint = action_hint(
    ...
    def test_click_success_gives_wait_hint(self):
        hint = action_hint(
            "desktop_control", {"action": "click File"},
            'Clicked "File" at (50,10).')
        assert "wait" in hint.lower()

    def test_click_send_to_opens_chat(self):
    ...
    def test_click_shows_send_to_card(self):
    ...
    def test_unrelated_command_no_hint(self):
        hint = action_hint("shell", {"command": "ls -la"}, "file1\nfile2")
        assert hint == ""

    def test_web_fetch_no_hint(self):
        hint = action_hint("web_fetch", {"url": "https://example.com"}, "page content")
        assert hint == ""
...
class TestBuildStepResults:
    ...
    def _make_state(self):
        ...
        from monad.cognition.reasoner import _SolveState
        return _SolveState()

    def test_empty_actions(self):
        state = self._make_state()
        assert state._build_step_results() == []

    def test_results_match_stored_data(self):
        state = self._make_state()
        state.actions.append({"capability": "web_fetch", "params": {"url": "https://x.com"}})
        state.actions.append({"capability": "python_exec", "params": {"code": "print(1)"}})
        state.task_state.store("web_fetch", "page content here")
        state.task_state.store("python_exec", "1\n")

        results = state._build_step_results()
        assert len(results) == 2
        assert results[0]["capability"] == "web_fetch"
        assert results[0]["result"] == "page content here"
        assert results[0]["success"] is True
        assert results[1]["capability"] == "python_exec"
        assert results[1]["result"] == "1\n"
        assert results[1]["success"] is True

    def test_error_result_marked_failed(self):
        state = self._make_state()
        state.actions.append({"capability": "shell", "params": {"command": "bad"}})
        state.task_state.store("shell", "[Error] command not found: bad")

        results = state._build_step_results()
        assert results[0]["success"] is False

    def test_missing_result_marked_failed(self):
        """When task_state has no matching key (e.g. action was intercepted)."""
        state = self._make_state()
        state.actions.append({"capability": "shell", "params": {"command": "open -a X"}})
        results = state._build_step_results()
    ...
    def test_params_included(self):
        state = self._make_state()
        state.actions.append({"capability": "web_fetch", "params": {"url": "https://x.com"}})
        state.task_state.store("web_fetch", "ok")
        results = state._build_step_results()
        assert results[0]["params"] == {"url": "https://x.com"}
