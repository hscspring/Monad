...
from monad.cognition.parser import clean_llm_output, parse_tags
from monad.config import TRUNCATE_MEDIUM, truncate
...
class Reflection:
    ...
    def learn(self, objective: dict, execution_result: dict) -> str:
        ...
        filepath = self.vault.save_record(
            task=task_desc, process=process,
            result=result_status, notes=summary,
        )
    ...
    @staticmethod
    def _build_prompt(objective: dict, execution_result: dict) -> str:
        ...
        for s in steps:
            ...
            step_details.append(
                f"  Step {s.get('step')}: {s.get('description')} [{status}] "
                f"→ {truncate(s.get('result', ''), TRUNCATE_MEDIUM)}"
            )
