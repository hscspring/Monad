...
- **protocol: pdf_generation**: New protocol guiding LLM to use `markdown_to_pdf` skill for PDF tasks (preferred), with reportlab manual fallback documented.
...
- **reasoner: task decomposition & plan tracking**: Before the ReAct loop, the Reasoner decomposes multi-step tasks into an explicit ordered plan via LLM. Each step maps to a skill or capability. The plan is injected as a checklist (`✅`/`⬜` with `← NEXT` pointer), updated after each action, and used for deterministic completion checking.

### Changed
- **desktop_control: knowledge extraction from reasoner**: All app-specific messaging rules (Feishu cmd+k, WeChat cmd+f, "发送给" panel, input box warnings, search result coordination) moved from hardcoded `REASONER_SYSTEM` prompt and `_action_hint` code to `knowledge/tools/desktop_control.md`. Loaded dynamically via `load_tools_docs()`. Reasoner hints are now generic (pattern-based), not app-specific.
- **reasoner: plan tracking with fuzzy match + LLM fallback**: `_update_plan` now handles hallucinated capability names (e.g. `send_wechat_message` when it should be `desktop_control`) via fuzzy matching against known skills/capabilities. Answer-time completion check falls back to LLM-based `_check_task_completion` when plan says incomplete but `desktop_control` was used.
- **reasoner: decompose_task failure logging**: `_decompose_task` now logs the specific error (e.g. `JSONDecodeError`) instead of silently falling back to single-step execution.
- **reasoner: plan decomposition prompt hardened**: `_PLAN_SYSTEM` now requires capability field to be an actual skill name or basic capability — no hallucinated names. Desktop UI tasks must use `desktop_control`.
- **publish_to_xhs: LLM content rewrite pipeline**: Raw article/blog content is now automatically rewritten for XHS audience. `_rewrite_for_xhs` converts to 200-400 char XHS-style body; updated `_CARD_SUMMARY_PROMPT` produces engaging text card copy. Eliminates "TL;DR" and blog artifacts.
- **reasoner: skills as direct capabilities**: LLM calls learned skills as `{"capability": "skill_name", "params": {...}}` instead of `python_exec` + manual imports. Fixes `sys.modules` caching conflicts and missing tool injection.
- **config: smart knowledge sync strategy**: Built-in knowledge in `skills/`, `protocols/`, `tools/` is always overwritten on startup. User-managed dirs use incremental sync.
- **reasoner: hollow answer guard → LLM-based completion check**: Replaced keyword-regex with semantic LLM validation. Fail-open, max 3 rejections.
- **record_screen skill removed**: Replaced by `start_recording` + `stop_recording` (two atomic skills).

### Fixed
- **skill_builder: LLM-based code review gate + desktop automation guard**: `_review_code()` semantically checks for hardcoded results, missing CJK fonts, wrong output paths, hollow logic, and now also rejects desktop automation skills (hardcoded `desktop_control()` + `time.sleep()` sequences). `SKILL_BUILDER_SYSTEM` explicitly skips desktop automation tasks and specific-entity analysis.
- **skill: removed hollow `send_wechat_message`**: Auto-generated skill that faked success without executing any desktop actions. Deleted from both project source and user workspace.
- **executor: centralized output file detection**: Any capability/skill creating files in `~/.monad/output/` now triggers `Output.file_link()` for the Web UI.
- **executor: inject `MONAD_OUTPUT_DIR` into skill modules**: Skills get correct `~/.monad/output/` path instead of falling back to home directory.
- **desktop_control: removed hardcoded app hints from tool output**: Auto-screenshot no longer appends "hotkey cmd f" — app-specific shortcuts are in the knowledge file.
- **reasoner: fix "发送给" hint loop**: Clicking "发送给百合" no longer triggers a hint to click it again. Distinguishes "appeared" vs "already clicked".
- **desktop_control: window bounds, OCR filtering, self-noise, search ambiguity**: Multiple fixes for window selection, element filtering, and click targeting.
- **gitignore: `.env.*` secret leak prevention**: Added `.env.*` pattern with `!.env.example` exception.
- **publish_to_xhs: smart title truncation**: Word-boundary aware truncation instead of hard cut.
